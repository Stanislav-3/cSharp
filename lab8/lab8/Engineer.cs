    ...
    public class Engineer: Student, ICompetition<Engineer>, IParty<Engineer>
    {
        public double Intelligence
        {
            ...
            private set
            {
                ...
                {
                    _intelligence = 0;
                    throw new Exception("Intelligence cannot be a negative value");
                }
                else if (_intelligence > 100)
                {
                    _intelligence = 100;
                    throw new Exception("Intelligence cannot be more than 100");
                }
                else
                {
        ...
        public void Party(int minutes)
        {
            ...
            double pIntelligence = Intelligence;
            double pPromise = Promise;
            Intelligence *= (1 - percent);
            Promise *= (1 - percent / 10);
            ParameterChangedMessage?.Invoke("Intelligence", pIntelligence, Intelligence);
            ParameterChangedMessage?.Invoke("Promise", pPromise, Promise);
        }

        public override void Study(int minutes, Transformer transformer)
        {
            base.Study(minutes, transformer);
            Random random = new Random();
            double pIntelligence = Intelligence;
            Intelligence += transformer(-3) * minutes * random.NextDouble();
            ParameterChangedMessage?.Invoke("Intelligence", pIntelligence, Intelligence);
        }
    }
}