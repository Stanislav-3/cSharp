    ...
    public class Humanitarian: Student, ICompetition<Humanitarian>, IParty<Humanitarian>
    {
        public double Vocabulary
        {
            ...
            private set
            {
                ...
                {
                    _vocabulary = 0;
                    throw new Exception("Vocabulary cannot be a negative value");
                }
                else if (_vocabulary > 100)
                {
                    _vocabulary = 100;
                    throw new Exception("Vocabulary cannot be more than 100");
                }
                else
                {
        ...
        public void Party(int minutes)
        {
            ...
            double pVocabulary = Vocabulary;
            double pPromise = Promise;
            Vocabulary *= (1 - percent);
            Promise *= (1 - percent / 10);
            ParameterChangedMessage?.Invoke("Vocabulary", pVocabulary, Vocabulary);
        ...
        public override void Study(int minutes, Transformer transformer)
        {
            ...
            Random random = new Random();
            double pVocabulary = Vocabulary;
            Vocabulary += transformer(-3) * minutes * random.NextDouble();
            ParameterChangedMessage?.Invoke("Vocabulary", pVocabulary, Vocabulary);
        }
