    ...
    public class Sportsman: Student, ICompetition<Sportsman>, IParty<Sportsman>, IConvertible
    {
        public double Strength
        {
            ...
            private set
            {
                ...
                {
                    _strength = 0;
                    throw new Exception("Strength cannot be a negative value");
                }
                else if (_strength > 100)
                {
                    _strength = 100;
                    throw new Exception("Strength cannot be more than 100");
                }
                else
                {
        ...
        public void Party(int minutes)
        {
            ...
            double pStrength = Strength;
            double pPromise = Promise;
            Strength *= (1 - percent);
            Promise *= (1 - percent / 10);
            ParameterChangedMessage?.Invoke("Strength", pStrength, Strength);
        ...
        public override void Study(int minutes, Transformer transformer)
        {
            ...
            Random random = new Random();
            double pStrength = Strength;
            Strength += transformer(-3) * minutes * random.NextDouble();
            ParameterChangedMessage?.Invoke("Strength", pStrength, Strength);
        }
